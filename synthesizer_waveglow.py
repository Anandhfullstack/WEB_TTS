...
class Synthesizer(object):
    def __init__(self, tts_checkpoint, tts_config, vocoder_checkpoint=None, vocoder_config=None, use_cuda=False, waveglow=False):
        ...
        self.load_tts(tts_checkpoint, tts_config,
                      use_cuda)
        if waveglow:
            self.load_vocoder_waveglow(vocoder_checkpoint, vocoder_config, use_cuda)
        elif vocoder_checkpoint:
    ...
    def load_vocoder_waveglow(self, model_file, model_config, use_cuda):
        self.vocoder_config = load_config(model_config)
        self.vocoder_ap = AudioProcessor(**self.vocoder_config['audio'])
        print(os.getcwd())
        print(model_file)
        waveglow = torch.load(model_file, map_location=torch.device('cpu'))['model']
        self.vocoder_model = waveglow
        # self.vocoder_model.load_checkpoint(self.vocoder_config, model_file, eval=True)
        if use_cuda:
            self.vocoder_model.cuda()
    ...
    def tts(self, text, speaker_idx=None):
        ...
        for sen in sens:
            ...
            if not use_gl:
                ...
                # [1, T, C]
                if self.waveglow:
                    waveform = self.vocoder_model.infer(vocoder_input.to(device_type), sigma=0.6)
                else:
