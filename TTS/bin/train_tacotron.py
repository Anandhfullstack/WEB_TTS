...
import TTS.io_again as io_utils
from TTS.utils.io import copy_model_files
from TTS.utils.radam import RAdam
from TTS.utils.tensorboard_logger import TensorboardLogger
import TTS.utils.training as training
...
def setup_loader(ap, r, is_val=False, verbose=False, dataset=None):
    if is_val and not c.run_eval:
        ...
        sampler = distribute.DistributedSampler(dataset) if num_gpus > 1 else None
        loader = DataLoader(
            dataset,
...
def train(data_loader, model, criterion, optimizer, optimizer_st, scheduler,
    ...
    model.train()
    epoch_time = 0
    keep_avg = gen_utils.KeepAverage()
    if use_cuda:
        batch_n_iter = int(
    ...
    for num_iter, data in enumerate(data_loader):
        ...
        if c.mixed_precision:
            ...
            optimizer, current_lr = training.adam_weight_decay(optimizer)
            ...
            if c.separate_stopnet:
                ...
                optimizer_st, _ = training.adam_weight_decay(optimizer_st)
        ...
        if num_gpus > 1:
            loss_dict['postnet_loss'] = distribute.reduce_tensor(loss_dict['postnet_loss'].data, num_gpus)
            loss_dict['decoder_loss'] = distribute.reduce_tensor(loss_dict['decoder_loss'].data, num_gpus)
            loss_dict['loss'] = distribute.reduce_tensor(loss_dict['loss'] .data, num_gpus)
            loss_dict['stopnet_loss'] = distribute.reduce_tensor(loss_dict['stopnet_loss'].data, num_gpus) if c.stopnet else loss_dict['stopnet_loss']
    ...
    if data_loader is not None:
        for num_iter, data in enumerate(data_loader):
            ...
            if num_gpus > 1:
                ...
                loss_dict['decoder_loss'] = distribute.reduce_tensor(loss_dict['decoder_loss'].data, num_gpus)
                if c.stopnet:
                    loss_dict['stopnet_loss'] = distribute.reduce_tensor(loss_dict['stopnet_loss'].data, num_gpus)
...
def main(args):  # pylint: disable=redefined-outer-name
    ...
    # DISTRUBUTED
    if num_gpus > 1:
        distribute.init_distributed(args.rank, num_gpus, args.group_id,
    ...
    # parse speakers
    num_speakers, speaker_embedding_dim, speaker_mapping = speakers.parse_speakers(c, args, meta_data_train, OUT_PATH)
    ...
    params = training.set_weight_decay(model, c.wd)
    ...
    if args.restore_path:
        ...
        try:
            ...
            model_dict = gen_utils.set_init_dict(model_dict, checkpoint['model'], c)
    ...
    # DISTRUBUTED
    if num_gpus > 1:
        model = distribute.apply_gradient_allreduce(model)

    if c.noam_schedule:
        scheduler = training.NoamLR(optimizer,
                           warmup_steps=c.warmup_steps,
                           last_epoch=args.restore_step - 1)
    else:
        scheduler = None

    num_params = gen_utils.count_parameters(model)
...
if __name__ == '__main__':
    ...
    if args.continue_path != '':
        ...
        print(args.continue_path)
        print(list_of_files)
        latest_model_file = max(list_of_files, key=os.path.getctime)
        args.restore_path = latest_model_file

    # setup output paths and read configs
    c = io_utils.load_configures(args.config_path)
    check_config_tts(c)
    ...
    if args.continue_path == '':
        OUT_PATH = io_utils.create_experiment_folder(c.output_path, c.run_name, args.debug)
    ...
    if args.rank == 0:
        ...
        new_fields["github_branch"] = gen_utils.get_git_branch()
        print(type(new_fields["github_branch"]))
        io_utils.copy_model(c,  args.config_path, OUT_PATH, new_fields)
    ...
    try:
        ...
        gen_utils.remove_experiment_folder(OUT_PATH)
        try:
            sys.exit(0)
        except SystemExit:
